...
const THERMOSTAT: Record<string, any> = {
    ...
    var request = http.get(options.host + "/sec/?pt=32", function (res: any) {
      ...
      res.on("end", () => {
        try {
          str = data.indexOf("<input name=misc size=4 value=");
          temp = data.substring(str + 30, 427);
            if (THERMOSTAT.TargetTemperature > 0) {THERMOSTAT.TargetHeatingCoolingState = 1;}
  ...
  setTargetHeatingCoolingState(state: CharacteristicValue) {
    ...
    if (state == 0) {
      var request = http.request(
        options.host + "/sec/?pt=32&misc=-50",
        function (res: any) {
          res.on("data", function (chunk: any) {
            data += chunk;
          });
      ...
      var request = http.request(
        options.host + "/sec/?pt=32&misc=" + THERMOSTAT.TargetTemperature,
        function (res: any) {
          res.on("data", function (chunk: any) {
            data += chunk;
          });
  ...
  setTargetTemperature(temperature: CharacteristicValue) {
    ...
    var request = http.get(
      options.host + "/sec/?pt=32&misc=" + temperature,
      function (res: any) {
        res.on("data", function (chunk: any) {
          data += chunk;
        });
...
ThermostatService
  ...
  .setCharacteristic(Characteristic.SerialNumber, "190420");
...
setInterval(() => {
  ...
  THERMOSTAT.getCurrentHeatingCoolingState();

ThermostatService.getService(Service.Thermostat)!.setCharacteristic(Characteristic.CurrentTemperature, THERMOSTAT.CurrentTemperature);
ThermostatService.getService(Service.Thermostat)!.setCharacteristic(Characteristic.CurrentHeatingCoolingState, THERMOSTAT.CurrentHeatingCoolingState);
}, 5000);
