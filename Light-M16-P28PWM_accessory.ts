...
class LightControllerClass {
  ...
  name: CharacteristicValue = "Чердак"; //name of accessory
  ...
  serialNumber: CharacteristicValue = "190121"; //serial number (optional)
  ...
  setPower(status: CharacteristicValue) {
    if ( status == "true" || status == 1 ) opts.path = "/sec/?cmd=28:" + LightController.brightness;
    else opts.path = "/sec/?cmd=28:0";
  ...
  getPower() { 
    opts.path = '/sec/?pt=28&cmd=get';
      ...
      let data = '';
       ...
       lightAccessory.getService(Service.Lightbulb)!.getCharacteristic(Characteristic.On)!.updateValue(false);}
    else {
       LightController.power = true;
       lightAccessory.getService(Service.Lightbulb)!.getCharacteristic(Characteristic.On)!.updateValue(true);}
      });
  });
  request.on('error', function (e) {
      console.log(e.message);
  ...
  setBrightness(brightness: CharacteristicValue) {
     opts.path = "/sec/?cmd=28:" + brightness;
    const request = http.request(opts, function (res) {
        let data = '';
        res.on('data', function (chunk) {
            data += chunk;
